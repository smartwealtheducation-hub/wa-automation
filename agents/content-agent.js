...
async function getLatestYouTubeVideo() {
  ...
  const url = `https://www.googleapis.com/youtube/v3/playlistItems?key=${YOUTUBE_API_KEY}&playlistId=${playlistId}&part=snippet&maxResults=1`;
  ...
  if (!data.items || data.items.length === 0) {
    console.log(
      `Uploads playlist (${playlistId}) returned no items - channel may have no public/unlisted videos, or the video is still processing.`
    );
    return null;
  }
  ...
  if (!videoId) {
    console.log(
      "Latest playlist item had no videoId - unexpected API response shape:",
      JSON.stringify(item)
    );
    return null;
  }
