...
// vars/secrets). If a category has no playlist configured, video selection
// falls back to the channel's general uploads instead of failing.
...
// Keeps only the most recent N video IDs, so older videos naturally
// re-enter rotation once a playlist has been fully cycled through instead
...
async function getLatestYouTubeVideo(category, usedVideoIds = []) {
  if (!YOUTUBE_API_KEY) {
    console.log("YOUTUBE_API_KEY not set, skipping video embed.");
    return null;
  }

  let playlistId = PLAYLIST_BY_CATEGORY[category?.name];

  if (playlistId) {
    console.log(`Using the "${category.name}" playlist for this article's video.`);
  } else {
    console.log(
      `No specific playlist configured for "${category?.name}", falling back to channel uploads.`
    );
  ...
  // Regular playlists (unlike the special "uploads" playlist) aren't
  // guaranteed to come back newest-first, so we pull a batch and sort by
  // publish date ourselves to find the actual most recent video.
  ...
  try {
    ...
    console.log("YouTube playlist fetch failed, skipping video embed:", err.message);
    return null;
  }

  if (!res.ok) {
    console.log(`YouTube playlistItems API returned ${res.status}, skipping video embed.`);
    return null;
  }
  ...
  if (!data.items || data.items.length === 0) {
    console.log(
      `Playlist (${playlistId}) returned no items - it may be empty or private.`
    );
    return null;
  }
  ...
  if (!candidates.length) {
    console.log(
      "Playlist items had no usable videoId/publishedAt fields - unexpected API response shape."
    );
    return null;
  }

  // Prefer the most recent video that hasn't been embedded recently, so a
  // small playlist doesn't just repeat its single newest video every time.
  const unused = candidates.filter((c) => !usedVideoIds.includes(c.videoId));
  if (!unused.length) {
    console.log(
      "Every recent video in this playlist has been used recently - reusing the most recent one to avoid leaving the article without a video."
    );
  }
  const chosen = unused.length ? unused[0] : candidates[0];
...
async function main() {
  ...
  if (hasPublishedToday(usedTopics)) {
    console.log(
      "An article was already published today - skipping this run to avoid a duplicate. (This is expected behavior when the 13:00 UTC retry run fires after a successful 09:00 UTC run.)"
    );
    return;
  }
  ...
  const video = await getLatestYouTubeVideo(category, usedVideos);
  console.log(
