...
// text-free and a crisp, real headline is composited on top afterward.
//
// The overlay is sized to the ACTUAL returned image dimensions (read via
// sharp's metadata()) rather than assumed request dimensions, since
// Pollinations doesn't always return exactly the requested width/height -
// sharp requires the overlay to exactly match or be smaller than the base
// image, so a mismatch here throws.
//
...
// Overlays a bold, legible headline + a dark gradient band onto a generated
// background image. Reads the image's REAL dimensions first (rather than
// assuming the requested ones) so the SVG overlay always matches exactly.
async function addTitleOverlay(imageBuffer, title) {
  ...
  return sharp(imageBuffer)
    .resize(width, height) // guarantees the base itself matches metadata exactly
...
blog.`;
  ...
  const withTitle = await addTitleOverlay(rawImage, title);
