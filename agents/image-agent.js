...
// whatever the headline actually needed, instead of overlapping it. Badge
// labels are shortened to fit the space between icons so they don't run
// into each other horizontally either.
//
...
async function addPosterOverlay(imageBuffer, title, badgeLabels, ctaText) {
  ...
  // Shrink the font somewhat for longer titles so more lines still fit
  // in a reasonable vertical space, up to 4 lines instead of 3.
  ...
  cursorY += Math.round(height * 0.11); // gap after headline before badges
  ...
  cursorY = badgeCenterY + badgeRadius + 24; // clears the badge label text
  cursorY += Math.round(height * 0.06); // gap before button
