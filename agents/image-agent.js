...
function iconBadgeSvg(cx, cy, radius, iconIndex, label, accentColor) {
  ...
  return `
    <g transform="translate(${cx}, ${cy})">
      ...
      <text x="0" y="${radius + 24}" font-family="Arial, Helvetica, sans-serif"
        font-weight="700" font-size="16" fill="white" text-anchor="middle">${escapeXml(label)}</text>
    </g>
  `;
}
...
async function addPosterOverlay(imageBuffer, title, badgeLabels, ctaText) {
  ...
  const badgeSpacing = Math.round(width * 0.15);
